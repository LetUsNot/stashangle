...
describe("domTargets marker form selectors", () => {
  ...
  it("collects all markers panel roots including event-key tab panes", () => {
    document.body.innerHTML = `
      <div class="nav-tabs">
        <a data-rb-event-key="scene-markers-panel">Markers</a>
      </div>
      <div class="scene-tabs">
        <div class="tab-pane" data-rb-event-key="scene-markers-panel">
          <div class="wall-item"><div class="wall-item-container"></div></div>
        </div>
      </div>
    `;

    const roots = findMarkersPanelRoots();
    expect(roots.length).toBeGreaterThan(0);
    expect(roots.some((root) => root.querySelector(".wall-item") !== null)).toBe(true);
  });
  ...
  it("finds placard host from marker preview media", () => {
    document.body.innerHTML = `
      <div class="wall-item">
        <div class="wall-item-container">
          <img src="/scene/15202/scene_marker/415/screenshot" />
        </div>
      </div>
    `;

    const host = findPlacardHostForMarkerId("415", document.body);
    expect(host?.className).toContain("wall-item-container");
  });
